...
class OrderModel {
  ...
  static async addItem(orderId, drinkId, quantity, unitPrice = null, discount = 0) {
    ...
    try {
      ...
      const { rows } = await client.query(
        `INSERT INTO order_detail (order_detail_id, order_id, drink_id, quantity, discount, total)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`,
        [orderDetailId, orderId, drinkId, quantity, discount, total]
      );
      return rows[0];
    } finally {
      client.release();
  ...
  static async getWithItems(orderId) {
    ...
    const { rows: items } = await pool.query(
      `SELECT od.order_detail_id, od.order_id, od.drink_id, d.name, d.price AS unit_price,
              od.quantity, od.discount, od.total
    ...
    try {
      ...
      for (const it of items) {
        ...
        await client.query(
          `INSERT INTO order_detail (order_detail_id, order_id, drink_id, quantity, discount, total)
           VALUES ($1, $2, $3, $4, $5, $6)`,
          [odId, orderId, drinkId, quantity, discount, total]
        );
      }
